...
class ResilientGraphMemory:
    ...
    def record_world(self, record: WorldMemoryRecord) -> GraphWriteReceipt:
        return self._write(record, self._cache.record_world, self._remote_method("record_world"))

    def record_obstacle(self, record: ObstacleMemoryRecord) -> GraphWriteReceipt:
        return self._write(
            record,
            self._cache.record_obstacle,
            self._remote_method("record_obstacle"),
        )
    ...
    def record_evaluated_policy(self, record: EvaluatedPolicyVersion) -> GraphWriteReceipt:
        return self._write(
            ...
            self._remote_method("record_evaluated_policy"),
        )
    ...
    def record_episode(self, record: EpisodeMemoryRecord) -> GraphWriteReceipt:
        return self._write(
            ...
            self._remote_method("record_episode"),
        )
    ...
    def record_failure(self, record: FailureMemoryRecord) -> GraphWriteReceipt:
        return self._write(
            ...
            self._remote_method("record_failure"),
        )
    ...
    def record_correction(self, record: CorrectionMemoryRecord) -> GraphWriteReceipt:
        return self._write(
            ...
            self._remote_method("record_correction"),
        )
    ...
    def record_lesson(self, record: LessonMemoryRecord) -> GraphWriteReceipt:
        return self._write(
            ...
            self._remote_method("record_lesson"),
        )
    ...
    def record_outperformance(self, record: PolicyComparisonRecord) -> GraphWriteReceipt:
        return self._write(
            ...
            self._remote_method("record_outperformance"),
        )
    ...
    def synchronize_local_cache(self) -> int:
        ...
        return self._cache.replay_to(self._remote)

    def _write[T: ContentAddressedRecord](
        self,
        record: T,
        cache_write: Callable[[T], GraphWriteReceipt],
        remote_write: Callable[[T], GraphWriteReceipt] | None,
    ) -> GraphWriteReceipt:
        ...
        try:
            remote_receipt = remote_write(record)
        except GraphProviderUnavailableError as exc:
        ...
        return GraphWriteReceipt(
            record_kind=remote_receipt.record_kind,
            record_id=remote_receipt.record_id,
            content_hash=remote_receipt.content_hash,
    ...
    def _remote_method[T: ContentAddressedRecord](
        self, method_name: str
    ) -> Callable[[T], GraphWriteReceipt] | None:
        if self._remote is None:
            return None
        method = getattr(self._remote, method_name)
        return cast(Callable[[T], GraphWriteReceipt], method)
